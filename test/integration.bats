...
@test "extract: https URL" {
    run bash -c 'echo "visit https://example.com today" | rg_extract'
    assert_success
    assert_output "https://example.com"
}

@test "extract: http URL" {
    run bash -c 'echo "visit http://example.com today" | rg_extract'
    assert_success
    assert_output "http://example.com"
}

@test "extract: ftp URL" {
    run bash -c 'echo "download ftp://files.example.com/file.tar.gz" | rg_extract'
    assert_success
    assert_output "ftp://files.example.com/file.tar.gz"
}

@test "extract: file URL" {
    run bash -c 'echo "open file:///home/user/doc.pdf" | rg_extract'
    assert_success
    assert_output "file:///home/user/doc.pdf"
}

@test "extract: URL with query and fragment" {
    run bash -c 'echo "https://example.com/path?q=1&r=2#section" | rg_extract'
    assert_success
    assert_output "https://example.com/path?q=1&r=2#section"
}

@test "extract: trailing period is stripped" {
    run bash -c 'echo "Visit https://example.com." | rg_extract'
    assert_success
    assert_output "https://example.com"
}

@test "extract: trailing comma is stripped" {
    run bash -c 'echo "See https://example.com, and more" | rg_extract'
    assert_success
    assert_output "https://example.com"
}

@test "extract: multiple URLs on one line" {
    run bash -c 'echo "https://a.com and https://b.com" | rg_extract'
    assert_success
...
@test "extract: GitHub shorthand with hyphens" {
    run bash -c "echo \"'my-org/my-repo'\" | rg_extract"
    assert_success
...
@test "extract: mixed URL types" {
    ...
    run bash -c "echo '$input' | rg_extract"
    assert_success
...
@test "extract: https://www not duplicated by www pattern" {
    run bash -c 'echo "https://www.example.com" | rg_extract'
    assert_success
    assert_output "https://www.example.com"
}

@test "extract: ANSI-wrapped URL is extracted" {
    run bash -c 'printf '"'"'\e[32mhttps://example.com\e[0m'"'"' | rg_extract'
    assert_success
    assert_output "https://example.com"
}

@test "extract: empty input produces no output" {
    run bash -c 'echo "" | rg_extract'
    assert_success
    assert_output ""
}
